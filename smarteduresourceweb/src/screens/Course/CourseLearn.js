    ...
    const loadLearnPage = async () => {
        ...
        try {
            ...
            try {
                ...
                const rawRooms = chatRes.data?.data ?? chatRes.data;
                const rooms = Array.isArray(rawRooms)
                    ? rawRooms
                    : rawRooms?.items || rawRooms?.content || rawRooms?.data || [];

                const courseRoom = rooms.find(r =>
                    Number(r.courseId) === Number(id) &&
                    String(r.type).toUpperCase() === "PRIVATE"
                );
                if (courseRoom) {
                                                ...
                                                onClick={async () => {
                                                    ...
                                                    try {
                                                        const res = await authApis().post(
                                                            endpoints["chat-private-room-by-course"](id)
                                                        );

                                                        const room = res.data?.data;

                                                        if (room?.id) {
                                                            nav(`/chat?courseId=${id}&type=PRIVATE&room=${room.id}`);
                                                        } else {
                                                            nav(`/chat?courseId=${id}&type=PRIVATE`);
                                                        }
                                                    } catch (err) {
                                                        console.error(err);
                                                        alert("Không tạo được phòng nhắn tin riêng với giảng viên.");
                                                    }
                                                }}
                                            >
